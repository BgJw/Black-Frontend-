...
import EmployeeDeleteModal from "../modals/RemoveEmployee";
...
const IconControls = () => {
    ...
    const disabled = status === Status.notFound || status === Status.loading || status === Status.error;

    return (
        <>
            <AddEmployee changeModal={addEmployeeModal.closeModal} isOpen={addEmployeeModal.isOpen} />
            <EmployeeDeleteModal changeModal={removeEmployeeModal.closeModal} isOpen={removeEmployeeModal.isOpen} />
            <center className="flex justify-end items-center h-10 gap-3 relative">
                ...
                <Tooltip content="Usuń pracownika" className={styleTooltip}>
                    <button
                        onClick={() => removeEmployeeModal.toggleModal()}
                        disabled={disabled}
                        className="disabled:opacity-20 disabled:cursor-default">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 hover:opacity-70">
